...
async def playwright_automation(page, itns_need_scraping):
    print(f"Processing {itns_need_scraping}")
    itns_with_no_data_on_Expeditors = []
    for itn in itns_need_scraping:
...
async def run_playwright(itns_need_scraping):
    async with async_playwright() as p:
        ...
        try:
            ...
            itns_with_no_data_on_Expeditors = await playwright_automation(page, itns_need_scraping)
            return itns_with_no_data_on_Expeditors

        finally:
            await context.close()

def run_scrape(itns_need_scraping):
    return asyncio.run(run_playwright(itns_need_scraping))