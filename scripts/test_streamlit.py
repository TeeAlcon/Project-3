...
def master_list_doc_ready(master_df: pd.DataFrame):
    ...
    df["Ready status"] = "not_ready"
    df.loc[has_at_least_one_doc & counts_match & swb_check, "Ready status"] = "ready"

    # Reason for not_ready
    df["Missing SLI, AVL, APL, Packing List"] = ~has_at_least_one_doc
    df["Number of document doesn't match"] = has_at_least_one_doc & ~counts_match
    ...
    col for col in [
        ITN_COL,
        "Missing SLI, AVL, APL, Packing List",
    ...
    not_ready_df = (
        df.loc[df["Ready status"] == "not_ready", error_cols]
        .drop_duplicates()
    ...
    return not_ready_df, not_ready_itns
...
def main():
    ...
    if page_key in AES_PAGE_FILES:
        ...
        if master_df.empty:
            ...
            not_ready_df, not_ready_itns = master_list_doc_ready(master_df)


            st.subheader("Not Ready ITNs")
            st.dataframe(not_ready_df, use_container_width=True)
            if not_ready_itns:
                st.warning(f"{len(not_ready_itns)} ITNs are not ready")
            else:
